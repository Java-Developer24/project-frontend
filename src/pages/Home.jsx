...
const Home = ({ serviceData }) => {
  ...
  const anyOtherService = Array.isArray(serviceData)
    ? serviceData.find((service) => service.name.toLowerCase() === "any other") || {}
    : {};
              ...
              {selectedService ? (
                selectedService.servers.map((server) => (
                  <button
                    ...
                    key={server._id || server.serverNumber} // Use _id or serverNumber for unique key
                    disabled={loading}
                    onClick={() =>
                ...
                filteredServices.map((i) => (
                  <button
                    className="bg-[#282828] py-4 px-3 md:px-5 flex mb-1 w-full items-center justify-between rounded-lg"
                    key={i._id || i.name} // Use _id if available
